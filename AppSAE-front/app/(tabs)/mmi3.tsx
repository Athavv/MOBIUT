...
export default function Mmi3Screen() {
  ...
  useEffect(() => {
    const load = async () => {
      setIsLoading(true);
      try {
        const [saes, projs] = await Promise.all([fetchSaes(), fetchProjectsByYear("MMI3")]);
        setAllSaes(saes.filter((s) => MMI3_SEMESTERS.includes(s.semester)));
        setProjects(projs);
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <View style={{ gap: 16, marginBottom: 8 }}>
              ...
              <View style={{ gap: 4 }}>
                <Text style={{ fontSize: 11, fontWeight: "700", color: colors.textMuted, letterSpacing: 1.5, textTransform: "uppercase" }}>MMI3</Text>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={{ gap: 4, marginBottom: 8 }}>
            ...
            <Text style={{ fontSize: 34, fontWeight: "800", color: colors.textPrimary, letterSpacing: -0.8 }}>MMI3</Text>
            <Text style={{ fontSize: 14, color: colors.textSecondary }}>
        ...
        ListEmptyComponent={
          <Text style={{ fontSize: 14, color: colors.textMuted }}>
            {isLoading ? "Chargement..." : "Aucune SAé S5/S6 disponible."}
          </Text>
        }
        renderItem={({ item: sae }) => {
